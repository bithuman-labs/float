...
DEFAULT_COOLDOWNS = {"mini_wave_hello": 3.0, "nod": 2.0, "smile": 1.5, "default": 2.0}
    ...
    class VideoControl:
        ...
        def __post_init__(self):
            ...
            if self.action:
                ...
                actions_to_check = (
                    [self.action] if isinstance(self.action, str) else self.action
                )
...
def _check_cooldown(gesture_name: str, cooldown: float) -> bool:
    ...
    if current_time - last_trigger < cooldown:
        ...
        logger.info(
            f"Gesture '{gesture_name}' still in cooldown ({remaining_cooldown:.1f}s remaining)"
        )
        return False
...
def _check_message_duplicate(message_id: str, dedup_window: float = 10.0) -> bool:
    ...
    # Clean old entries
    expired_keys = [
        k for k, v in _processed_messages.items() if current_time - v > dedup_window
    ]
    ...
    if message_id in _processed_messages:
        ...
        if current_time - last_processed < dedup_window:
            logger.info(
                f"Duplicate message detected: {message_id} (processed {current_time - last_processed:.1f}s ago)"
            )
            return True
...
async def trigger_gesture_with_debounce(
    runtime: Any,
    action: str,
    target_video: Optional[str] = None,
...
async def trigger_wave_hello(
    runtime: Any, cooldown: Optional[float] = None, message_id: Optional[str] = None
) -> bool:
    """Trigger a mini wave hello animation with debounce protection"""
    return await trigger_gesture_with_debounce(
        runtime, "mini_wave_hello", cooldown=cooldown, message_id=message_id
    )


async def trigger_nod(
    runtime: Any, cooldown: Optional[float] = None, message_id: Optional[str] = None
) -> bool:
    """Trigger a nod animation with debounce protection"""
    return await trigger_gesture_with_debounce(
        runtime, "nod", cooldown=cooldown, message_id=message_id
    )


async def trigger_smile(
    runtime: Any, cooldown: Optional[float] = None, message_id: Optional[str] = None
) -> bool:
    """Trigger a smile expression with debounce protection"""
    return await trigger_gesture_with_debounce(
        runtime, "smile", cooldown=cooldown, message_id=message_id
    )


async def trigger_custom_action(
    runtime: Any,
    action: str,
    target_video: Optional[str] = None,
    cooldown: Optional[float] = None,
...
def get_gesture_cooldown_status(gesture_name: str) -> Dict[str, Union[bool, float]]:
    ...
    return {
        ...
        "is_available": last_trigger == 0
        or time_since_last > 0,  # Always available if no default cooldown
