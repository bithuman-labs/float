...
class BaseOptions:
    ...
    def initialize(self, parser):
        ...
        # video
        parser.add_argument(
            "--input_size", type=int, default=512, help="input image size"
        )
        parser.add_argument(
            "--input_nc", type=int, default=3, help="input image channel"
        )
        ...
        parser.add_argument(
            ...
            type=int,
            default=2,
            help="number of adjacent frames. For value v, t -> [t-v, ..., t, ..., t+v]",
        )
        parser.add_argument(
            "--wav2vec_sec",
            default=2,
            type=float,
            help="window length L (seconds), 50 frames",
        )
        parser.add_argument(
            "--wav2vec_model_path",
            default=os.path.join(checkpoint_base, "wav2vec2-base-960h"),
        )
        parser.add_argument(
            "--audio2emotion_path",
            default=os.path.join(
                checkpoint_base, "wav2vec-english-speech-emotion-recognition"
            ),
        )
        parser.add_argument(
            "--attention_window",
            default=2,
            type=int,
            help="attention window size, e.g., if 1, attend frames of t-1, t, t+1 for frame t",
        )

        parser.add_argument("--only_last_features", action="store_true")
        parser.add_argument(
            "--average_emotion", action="store_true", help="averaging emotion or not."
        )
        ...
        parser.add_argument(
            "--style_dim", type=int, default=512, help="w latent dimension"
        )
        ...
        parser.add_argument(
            "--dim_m", type=int, default=20, help="dimension of orthogonal basis"
        )
        ...
        parser.add_argument(
            ...
            default=1,
            type=float,
            help="max grad norm for training transformers",
        )
        ...
        parser.add_argument(
            "--torchdiffeq_ode_method", default="euler", help="ODE solver"
        )
        parser.add_argument(
            "--a_cfg_scale",
            default=2.0,
        ...
        parser.add_argument(
            "--n_diff_steps", type=int, default=500, help="number of diffusion steps"
        )
        parser.add_argument(
            "--diff_schedule",
            type=str,
        ...
        parser.add_argument(
            "--diffusion_mode", type=str, default="sample", choices=["sample", "noise"]
        )
        return parser
